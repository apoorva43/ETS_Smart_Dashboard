...
On the statistical side, the dashboard follows PISA methodology correctly. Weighted percentiles are averaged across all ten plausible values [@oecd_pisa_2022_technical] using `DescrStatsW` with the `W_FSTUWT` sampling weight. Standard errors account for both sampling variance from the 80 Fay BRR replicate weights and imputation variance across plausible values [@oecd_pisa_2022_technical]. 
